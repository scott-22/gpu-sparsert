...
for A_blocks in $(($A_dim / 8)); do
	...
	for C_blocks in $((C_dim / 49)); do
		for Gy in 1; do
            ...
            nvcc sparsednn/driver_spmm.cpp -w -O3 -DA_dim=$A_dim,B_dim=$B_dim,C_dim=$C_dim,A_Blocks=$A_blocks,C_Blocks=$C_blocks,Gy=$Gy,infile=$infile_str -lcuda -lcudart -lcnpy -lcublas -o exe --std=c++11 -Xptxas="-v" -I /h/s29hao/gpu-sparsert/build/include -L /h/s29hao/gpu-sparsert/build/lib -L /pkgs/cuda-11.0/lib64
			./exe > runtime
			cat runtime
